...
const containerSelector = '.lrud-container';
...
// Enable these later when we refactor for performance optimizations
// const isContainer = (node) => node?.classList?.contains('lrud-container');

// const isContainerWithAutofocus = (node) =>
//   node?.classList?.contains('lrud-container') &&
...
export const updateAncestorsAutoFocus = (elem, scope) => {
  // Traverse up the DOM tree to find all ancestor .lrud-container with data-autofocus="true"
  const ancestorContainers = [];
  let current = elem.parentElement;
  while (current && current !== scope || current.tagName !== 'BODY') {
    if (matches(current, '.lrud-container[data-autofocus="true"]')) {
      ancestorContainers.push(current);
    }
    current = current.parentElement;
  }

  // Update each ancestor container with the last focused child's id
  ancestorContainers.forEach((container) => {
      container.setAttribute(DATA_FOCUS_ATTRIBUTE, elem.id);
  });
};
