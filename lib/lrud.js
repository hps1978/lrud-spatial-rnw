 ...
 * 2. TVFocusGuideView.focusable={false | undefined} -> set .lrud-ignore
 * 3. TVFocusGuideView.autoFocus={true} -> TODO: predictable focus behaviour: first focusable child and remember last
 *    focused child (using data-focus attribute in parent containers). The data-focus stores the value of lrud-id attribute.
...
const matches = (element, selectors) => {
  if (!element) {
    console.warn('matches() passed with null element');
    return false;
  }
  ...
  for (const candidate of focusableCandidates) {
    ...
    if (!isCurrentContainer && (!isNestedContainer || candidateIsContainer)) {
      ...
      if (candidateContainer && !isAnscestorContainer) {
        ...
        if (matches(newFocus, containerSelector)) {
          // If the selected one is a another Container
          continue;
        }
