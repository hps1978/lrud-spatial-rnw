...
export const getNextFocus = (elem, keyOrKeyCode, scope) => {
  ...
  const blockExit = parentContainer?.getAttribute(PRIORITISE_CHILDREN_ATTRIBUTE)?.valueOf() === 'true' && scope.contains(parentContainer);

  const candidates = getAllFocusables(scope);
  focusableCandidates = sortValidCandidates(candidates, elem, exitDir);
  nextFocus = getNextFromCandidates(parentContainer, focusableCandidates, exitDir, blockExit);
  if (nextFocus) {
    return nextFocus;
  }
  // If here nothing could be found!!
