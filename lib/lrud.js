...
export const getNextFocus = (elem, keyOrKeyCode, scope) => {
  ...
  const screenParent = elem && elem.closest(`[id^="${LRUD_SCREEN_ID_PREFIX}"]`);
  let scopedContainer = screenParent || ((!scope || !scope.querySelector) ? document.body : scope);

  // If no element is currently focused, get default focus in the scope (if any) and return
  if (!elem) return getDefaultFocus(scopedContainer, exitDir);
  ...
  let candidates = [];
  const focusables = getAllFocusables(elem, scopedContainer);
