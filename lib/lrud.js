...
const getParentContainerWithBlockedExits = (elem, exitDir, scope) => {
  ...
  if (!parent || parent === scope || parent.tagName === 'BODY') {
    return null ;
  } else if (isContainer(parent) && blockExits?.length > 0 && blockExits.includes(exitDir)) {
    return parent;
  }
 ...
 * @return {{elem: HTMLElement|null, parentHasAutofocus: boolean}} Next focus element
 */
  ...
  for (const candidate of focusables) {
    if (isInViewportCallback(candidate)) {
      const parentHasAutofocus =
        getFocusableParentContainer(candidate)?.getAttribute(DATA_AUTOFOCUS_ATTRIBUTE) ===
          'true' || false;
