...
export const getNextFocus = (elem, keyOrKeyCode, scope) => {
  ...
  // candidates can be sciped to the ones in the container
  ...
  // Refine the scope:
  // 1. If exits are blocked, scope is the parent container with block exit.
  // 2. If current element is provided, check if there is a parent with id that matches LRUD_SCREEN_ID_PREFIX<string.
  // 3. Default to the provided scope.
  const scopedContainer = exitsBlocked ? originsBlockedInfo.parent : elem.closest(`[id^="${LRUD_SCREEN_ID_PREFIX}"]`) || scope;
