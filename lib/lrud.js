...
const matchesFunction = Element.prototype.matches ||
 ...
 * @return {HTMLElement|null} First focusable HTML Element
 */
const getDefaultFocus = (scope, exitDir) => {
  let nextFocus = {focus: null, parentHasAutofocus: false};
  ...
  if (candidates.length > 0) {
    ...
    if (candidateIsContainer) {
      ...
      nextFocus = { focus: candidate, parentHasAutofocus: false };
    }
  }
  return nextFocus;
};
 ...
 * @return {{destination: HTMLElement|null, parentHasAutofocus: boolean}  } Next focus element in container
 */
const findDestinationOrAutofocus = (elem, exitDir, candidateContainer) => {
  ...
  if (destination !== undefined && destination !== null) {
      return { focus: destination, parentHasAutofocus: hasAutoFocus };
  }
  ...
  if (hasAutoFocus) {
    ...
    if (newFocus) {
      return { focus: newFocus, parentHasAutofocus: hasAutoFocus };
    }
  ...
  return { focus: null, parentHasAutofocus: hasAutoFocus };
};
 ...
 * @return {{focus: HTMLElement|null, parentHasAutofocus: boolean}} candidate that gets the next focus or null if nothing valid found
 */
...
const getNextFromCandidates = (focusableCandidates, elem, exitDir) => {
  ...
  for (const candidate of focusableCandidates) {
    ...
    if (!allowBlockExits) {
      if (!candidateIsContainer) {
        ...
        if (candidatesContainer === originContainer) {
          ...
          return { focus: candidate, parentHasAutofocus };
        }
      }
      // Trying to enter a different container, exit is blocked, return null
      return { focus: null, parentHasAutofocus: false };
    }
    ...
    if (candidateIsContainer) {
      ...
      nextFocus = { focus: candidate, parentHasAutofocus };
    }
  }
