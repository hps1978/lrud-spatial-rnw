...
export const getNextFocus = (elem, keyOrKeyCode, scope) => {
  ...
  // If current element is provided, check if there is a parent with id that matches LRUD_SCREEN_ID_PREFIX<string>
  // to define the scope of search. This is to handle cases where there are multiple separate focus trees in the DOM (e.g. multiple screens in a TV app)
  const screenContainer = elem.closest(`[id^="${LRUD_SCREEN_ID_PREFIX}"]`) || scope;
  let nextFocus = null;

  let candidates = [];
  const focusables = getAllFocusables(elem, screenContainer);
