 ...
 * @return {HTMLElement|null} Next focus element in container
 */
const findAutofocus = (candidateContainer) => {
 ...
 *
 * @param {HTMLElement} searchOriginsParent Parent of the starting element (current starting point)
...
const getNextFromCandidates = (searchOriginsParent, focusableCandidates, exitDir, blockSetExits) => {
  for (const candidate of focusableCandidates) {
    const candidateIsContainer = matches(candidate, containerSelector);
    const candidatesParent = getParentContainer(candidate);
    const candidateContainer = candidateIsContainer ? candidate : candidatesParent;

    const isCurrentContainer = candidateContainer === searchOriginsParent;
    const isNestedContainer = searchOriginsParent?.contains(candidateContainer);
    const isAnscestorContainer = candidateContainer?.contains(searchOriginsParent);
    ...
    if (!isCurrentContainer && (!isNestedContainer || candidateIsContainer)) {
      if (blockSetExits) {
        const blockedExitDirs = getBlockedExitDirs(searchOriginsParent, candidateContainer);
      ...
      if (candidateContainer && !isAnscestorContainer) {
        if (candidateIsContainer && candidateContainerAutoFocus) {
          const nextFocus = findAutofocus(candidateContainer);
          if (nextFocus) {
            return nextFocus;
          }
          // Nothing in this container candidate, try the next candidate
          continue;
        } else {
          console.log('Else of: candidateIsContainer && candidateContainerAutoFocus: ', candidate);
          // Check for Parent, of not fall through to use the candidate as next focus
          if (candidatesParent && candidateContainerAutoFocus) {
            const nextFocus = findAutofocus(candidateContainer);
            if (nextFocus) {
              return nextFocus;
            }
          }
          // Fall through as the candidate may be returned as focus
        }
      } else {
    ...
    if (!candidateIsContainer && candidateContainerAutoFocus) { // TODO: Check if candidateContainerAutoFocus is needed here
